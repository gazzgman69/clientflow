...
export function DraftReplyButton({ emailId, onDraftGenerated }: DraftReplyButtonProps) {
  ...
  const toneMutation = useMutation({
    mutationFn: async (tone: string) => {
      const response = await apiRequest('POST', `/api/ai/emails/${emailId}/adjust-tone`, { tone });
      const data = await response.json();
      return data;
    },
    onSuccess: (data) => {
      ...
      toast({
        title: "Tone adjusted",
        description: "Draft regenerated with new tone",
      });
    },
    onError: (error: Error) => {
      toast({
          ...
          <div className="space-y-4">
            {draftContent ? (
              <>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tones.map((tone) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        onClick={() => toneMutation.mutate(tone.value)}
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-muted-foreground">
                    Generated by {draftMutation.data?.model || 'gpt-4o-mini'}
                  </div>
                  <Button
                    variant="secondary"
                    size="sm"
